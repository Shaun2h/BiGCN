...
def loadBiData(dataname, treeDic, fold_x_train, fold_x_test, TDdroprate,BUdroprate,picklefear=True):
    data_path = os.path.join(cwd,'data', dataname + 'graph')
    print("loading train set", )
    traindata_list = BiGraphDataset(fold_x_train, treeDic, tddroprate=TDdroprate, budroprate=BUdroprate, data_path=data_path,ispheme=dataname=="PHEME",picklefear=picklefear)
    print("train no:", len(traindata_list))
    print("loading test set", )
    testdata_list = BiGraphDataset(fold_x_test, treeDic, data_path=data_path,ispheme=dataname=="PHEME",picklefear=picklefear)
    print("test no:", len(testdata_list))
    return traindata_list, testdata_list
